...
Client.prototype.call = function(method, params, callback, errback, path) {
  ...
  let cbCalled = false;
  // use AbortController to cancel the request on timeout (request.abort() is deprecated)
    ...
    response.on('end', function() {
      ...
      decoded.forEach(function(decodedResponse, i) {
        if (decodedResponse.hasOwnProperty('error') && decodedResponse.error != null) {
          ...
          // Response has neither 'result' nor 'error' — malformed JSON-RPC response.
          // decodedResponse.error is undefined here, so we must NOT access .message/.code on it.
          if (errback) {
            err = new Error('Invalid JSON-RPC response: neither result nor error provided');

            if (decodedResponse.error && decodedResponse.error.code)
              err.code = decodedResponse.error.code;
