...
function find_address(hash, caseSensitive, cb) {
  if (typeof hash !== 'string') return cb();
  if (caseSensitive) {
    // faster search but only matches exact string including case
    Address.findOne({a_id: {$eq: hash}}).then((address) => {
      if (address)
        return cb(address);
      else
        return cb();
    ...
    Address.findOne({a_id: {$regex: '^' + safeHash + '$', $options: 'i'}}).then((address) => {
      if (address)
        return cb(address);
      else
        return cb();
...
function find_claim_name(hash, cb) {
  if (typeof hash !== 'string') return cb();
  ClaimAddress.findOne({a_id: {$eq: hash}}).then((claim_address) => {
    if (claim_address)
...
module.exports = {
  ...
  connect: function(database, cb) {
    ...
    mongoose.connect(database).then(() => {
      ...
      // NOTE: do not log err directly as it may contain the connection URI with credentials
  ...
  update_claim_name: function(hash, claim_name, cb) {
    if (typeof hash !== 'string' || typeof claim_name !== 'string') return cb('invalid_input');
    ...
    module.exports.get_address(hash, false, function(address) {
      ...
      if (address) {
        ...
        if (claim_name == null || claim_name == '') {
          ...
          ClaimAddress.findOneAndDelete({a_id: {$eq: hash}}).then(() => {
          ...
          // $set updates claim_name; $setOnInsert populates a_id only on upsert insert.
          // Both hash and claim_name are already validated as strings (typeof guard above).
          ClaimAddress.updateOne({a_id: {$eq: hash}}, {
            $set: {claim_name: claim_name},
            $setOnInsert: {a_id: hash}
          }, {
            upsert: true
          }).then(() => {
            // run processes after the claim name has been updated
    ...
    if (settings.masternodes_page.enabled == true) {
      ...
      module.exports.get_masternodes(function(masternodes) {
        ...
        if (updated) {
          ...
          // $set is required: without it Mongoose sends a replacement document
          // that would erase all other Masternode fields (rank, txhash, ip_address, etc.)
          Masternode.updateOne({addr: {$eq: hash}}, {
            $set: {claim_name: claim_name}
          }).then(() => {
            // finished updating the claim label
            return cb('');
  ...
  get_last_txs_ajax: function(start, length, min, cb) {
    ...
    if (min > 0) {
      ...
      Stats.findOne({coin: settings.coin.name}).then((stats) => {
        // get last transactions where there is at least 1 vout
        Tx.find({'total': {$gte: min}, 'vout': { $gte: { $size: 1 }}}).sort({blockindex: -1}).skip(Number(start)).limit(Number(length)).exec().then((txs) => {
          return cb(txs, (stats && stats.txes ? stats.txes : 0));
        }).catch((err) => {
          console.log(err);
          return cb([], 0);
        });
              ...
              if (blockhash) {
                ...
                lib.get_block(blockhash, function(block) {
                  if (block) {
                    ...
                    let newNetworkHistory = new NetworkHistory({
                      blockindex: height,
  ...
  remove_sync_message: function() {
    let filePath = './tmp/show_sync_message.tmp';
