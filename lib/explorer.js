...
// Replacement for the deprecated postman-request package.
// All web API calls in this file follow the same pattern:
//   request({uri: url, json: true}, function(error, response, body) { ... })
// This helper replicates that callback signature using native fetch (Node.js 18+).
function request(options, cb) {
  fetch(options.uri)
    .then(function(res) { return res.json(); })
    .then(function(body) { cb(null, null, body); })
  ...
  if (typeof first_vout !== 'undefined' && first_vout.scriptPubKey.type == 'nonstandard') {
    if (arr_vin.length > 0 && arr_vout.length > 0) {
      if (arr_vin[0].addresses == arr_vout[0].addresses) {
        // pos
        arr_vout[0].amount = arr_vout[0].amount.sub(arr_vin[0].amount);
        arr_vin.shift();
...
module.exports = {
  ...
  create_lock: function(lock) {
    ...
    const fname = path.join(path.dirname(__dirname), 'tmp', `${lock}.pid`);
  ...
  is_locked: function(lock_array, silent = false) {
    ...
    for (let i = 0; i < lock_array.length; i++) {
      ...
      if (fs.existsSync(pidFile)) {
        ...
        // the pid file exists; read and validate the pid as a positive integer
        const rawPid = fs.readFileSync(pidFile).toString().trim();
        const pid = parseInt(rawPid, 10);

        if (!Number.isInteger(pid) || pid <= 0) {
          // malformed pid file — treat lock as stale
          deactivateLock = true;
        } else {
          // determine the operating system
          switch (process.platform) {
            case 'win32': {
              // windows: use tasklist with the validated integer pid
              const { execSync } = require('child_process');
              try {
                const cmdResult = execSync(`tasklist /FI "PID eq ${pid}"`);
                // crude check: fewer than 4 lines means no match; node.exe must appear
            ...
            default:
              // linux or other: use process.kill(pid, 0) — no shell, no injection risk.
              // signal 0 tests for process existence without sending a real signal.
              // throws ESRCH if the process does not exist, EPERM if it exists but
              // we lack permission (process is still running in that case).
              try {
                process.kill(pid, 0);
                // no throw means the process is running — lock is still valid
              } catch (err) {
                if (err.code === 'ESRCH') {
                  // process does not exist — deactivate the stale lock
                  deactivateLock = true;
                }
                // EPERM: process exists but belongs to another user — keep lock active
              }
          }
        }
