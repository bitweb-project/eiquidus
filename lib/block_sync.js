  ...
  delete_and_cleanup_tx: function(txid, block_height, timeout, cb) {
    ...
    AddressTx.find({txid: txid}).exec().then((address_txes) => {
      if (address_txes.length == 0) {
        ...
        Tx.findOne({txid: txid}).then((tx) => {
          let addressTxArray = [];
          let has_vouts = (tx.vout != null && tx.vout.length > 0);
          ...
          if (tx.vin != null && tx.vin.length > 0) {
            ...
            for (let vin_tx_counter = tx.vin.length - 1; vin_tx_counter >= 0; vin_tx_counter--) {
              // loop through the addresstx data
              for (let vin_addresstx_counter = address_txes.length - 1; vin_addresstx_counter >= 0; vin_addresstx_counter--) {
          ...
          if (tx.vout != null && tx.vout.length > 0) {
            ...
            for (let vout_tx_counter = tx.vout.length - 1; vout_tx_counter >= 0; vout_tx_counter--) {
              // loop through the addresstxe data
              for (let vout_addresstx_counter = address_txes.length - 1; vout_addresstx_counter >= 0; vout_addresstx_counter--) {
                // check if there is a tx record that exactly matches to the addresstx
