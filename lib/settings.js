...
exports.shared_pages = {
  ...
  "page_footer": {
    ...
    "powered_by_text": "<a class='nav-link poweredby' href='https://github.com/bitweb-project/eiquidus' target='_blank'>eIquidus Explorer v{explorer_version}</a>"
  }
};
...
exports.markets_page = {
  ...
  //            Supported exchanges: dex-trade, dexomy, freiexchange/freixlite, nestex, nonkyc, poloniex, yobit
  "exchanges": {
...
// Keys that must never be assigned/traversed to prevent prototype pollution.
const FORBIDDEN_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

/**
 * Set a nested property in an object using a dot-separated path.
...
function setDeepProperty(obj, path, value) {
  ...
  // FORBIDDEN_KEYS blocks __proto__/constructor/prototype traversal above;
  // the typeof guard below ensures current is still a plain object before writing,
  // satisfying CodeQL's js/prototype-pollution-utility structural check.
  if (!FORBIDDEN_KEYS.has(lastKey) && current !== null && typeof current === 'object')
    current[lastKey] = value;
}

/**
 * Delete a nested property given a dot-separated path.
 */
...
/**
 * Ensure that the object hierarchy exists for a given dot-separated path.
 */
function ensureParentPath(obj, path) {
  const keys = path.split('.');
  let current = obj;
  for (let i = 0; i < keys.length - 1; i++) {
    const key = keys[i];
...
exports.loadSettings = function loadSettings() {
  ...
  if (json_settings != null) {
    ...
    for (const current_setting in json_settings) {
    ...
    for (const current_setting in exports) {
...
const fix_deprecated_setting = function(json_settings, old_path, new_path) {
  const oldValue = Object.byString(json_settings, old_path);
  if (oldValue !== undefined) {
    // new setting does not exist yet -> migrate
    if (Object.byString(json_settings, new_path) === undefined) {
      // remove old setting
      deleteNestedProperty(json_settings, old_path);
      // ensure parent structure exists
      ensureParentPath(json_settings, new_path);
      // copy value (deep copy to avoid reference issues)
      setDeepProperty(json_settings, new_path, JSON.parse(JSON.stringify(oldValue)));
      // warning message
...
const ensure_parent_elements_exist = function(json_settings, path) {
  ...
  if (split.length > 1) {
    ...
    for (let i = 0; i < split.length - 1; i++) {
      ...
      let current_value = Object.byString(json_settings, running_path)
      ...
      if (current_value == null || typeof current_value !== 'object') {
        ...
        setDeepProperty(json_settings, running_path, {});
      }
    } 
  }
...
const merge_settings = function(json_settings, exceptions, current_setting, path) {
  ...
  if (typeof current_setting === 'object' && current_setting !== null) {
    ...
    if (Object.byString(exports, path) == null) {
      ...
      if (Array.isArray(current_setting) || exceptions.indexOf(path) > -1) {
        // merge the whole object/array without checking individual keys
        setDeepProperty(exports, path, JSON.parse(JSON.stringify(Object.byString(json_settings, path))));
      } else {
        for (const setting_name in current_setting) {
    ...
    if (Object.byString(exports, path) == null) {
      ...
      const setting_value = Object.byString(json_settings, path);
      setDeepProperty(exports, path, JSON.parse(JSON.stringify(setting_value)));
    }
  }
}

// define a recursive function used to check settings for missing entries between json objects
const check_missing_settings = function(json_settings, exceptions, current_setting, path) {
  // check if this is an object with more properties
  if (typeof current_setting === 'object' && current_setting !== null) {
    // this is an object
